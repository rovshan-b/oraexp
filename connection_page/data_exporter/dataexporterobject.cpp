...
DataExporterObject::DataExporterObject(DataExporterBase *exporter,
...
DataExporterObject::~DataExporterObject()
{
    if(!bulkMode){
        delete exporter;
    }
...
void DataExporterObject::exportData()
{
    QString errMessage;
...
void DataExporterObject::stop()
{
    this->stopped = true;
}

void DataExporterObject::setOptions(IQueryScheduler *queryScheduler, const QString &schemaName, const QString &tableName, DataExporterOptions *options, const TableInfoForDataComparison &tableOptions)
{
    this->bulkMode=true;
...
Statement *DataExporterObject::getStatement()
{
...
void DataExporterObject::exportToStream(QTextStream &out)
{
    int exportedCount = 0;
    ...
    if(exporter->includeColumnHeaders){
        QStringList columnTitles = exporter->getColumnTitles();
