declare
  ...
  l_scan_type number := :scan_type; --0 - dependents, 1 - references, 2 - dependents and references 
  ...
  c_dependent_obj_query constant varchar2(4000) := 'select name, type from sys.all_dependencies where owner = :owner and referenced_owner = :owner and referenced_name = :object_name and referenced_type = :object_type';
  c_referenced_obj_query constant varchar2(4000) := 'select referenced_name as name, referenced_type as type from sys.all_dependencies where owner = :owner and referenced_owner = :owner and name = :object_name and type = :object_type';
  c_both_query constant varchar2(4000) := c_dependent_obj_query || ' union ' || c_referenced_obj_query;
                     
  c_dependent_tables_query constant varchar2(4000) := 'select table_name as name, ''TABLE'' as type
  ...
  c_referenced_tables_query constant varchar2(4000) := 'select (select table_name from sys.all_constraints where owner=c1.r_owner and constraint_name=c1.r_constraint_name) as name, ''TABLE'' as type
  ...
  function already_collected(p_obj_name varchar2, p_obj_type varchar2) return number is
  begin
  ...
  procedure add_to_collected(p_obj_name varchar2, p_obj_type varchar2) is
  begin
     if p_obj_name is null then
        return;
     end if;
     ...
     if p_obj_name like l_name_like and p_obj_name not like l_name_not_like and is_valid_type(p_obj_type)=1 then
        l_result := l_result || p_obj_name||','||p_obj_type||','||l_current_level||';';
     end if;
  end;
  ...
  begin
      ...
      LOOP 
          if dbms_sql.fetch_rows(l_cursor_id)>0 then 
            ...
            if already_collected(l_name, l_type) = 0 and (l_stop_on_invalid_type = 0 or is_valid_type(l_type) = 1) then
               add_to_collected(l_name, l_type);
               add_to_queue(l_name, l_type);
               
               if l_type = 'PACKAGE' then
                  add_to_collected(l_name, 'PACKAGE BODY');
                  add_to_queue(l_name, 'PACKAGE BODY');
               elsif l_type = 'TYPE' then
                  add_to_collected(l_name, 'TYPE BODY');
