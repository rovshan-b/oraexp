    ...
    begin     
      if p_hash = create_source_hash(p_object_type, p_owner, p_object_name, p_owner /*if we use p_new owner here resulting hashes will always be different*/, p_remove_whitespace, p_wrap) then
        return 'same';
      else
        return 'different';
      end if;
